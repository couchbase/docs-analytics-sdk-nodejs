...
A connection to an Analytics cluster is represented by an `analytics` object.
...
Connect to Enterprise Analytics by calling `analytics.createInstance` with the connection string, credentials, and any required optional parameters.
...
async function main() {
    ...
    const cred = Credential.from_jwt(_EXAMPLE_JWT)
    const cluster = Cluster.create_instance(endpoint, cred)
----
